...
public class DESUtil {
	...
	public static String changePin(String pin, String cardNo, String PinKey, String termKeyIndex, String bankKey,
			String bankindex) throws Exception {
		...
		String bankeyKeyPro = prop.getProperty(bankindex);
		String srcKeyPro = prop.getProperty(termKeyIndex);
		// 解析密钥明文
		String srcKeyDec = bcd2Str(DESUtil.decrypt3(PinKey, srcKeyPro));
		
		String pinMingWen = bcd2Str(DESUtil.decrypt3(pin, srcKeyDec));
		
		String bankkey = bcd2Str(DESUtil.decrypt3(bankKey, bankeyKeyPro));
		
		String pinMiWeb = bcd2Str(DESUtil.encrypt3(pinMingWen, bankkey)); 
		
		return pinMiWeb;
//		// 解析银联密钥明文
//		byte[] pinKeyByte = DESUtil.decrypt3(bankKey, bankeyKeyPro);
//		// 解析密码明文
//		byte[] bytes1 = DESUtil.decrypt3(pin, srcKeyDec);
//		byte[] pwdByte = new byte[8];
//		int cardLen = cardNo.length();
//		String pan = "0000" + cardNo.substring(cardLen - 13, cardLen - 1);
//		byte[] bytes2 = ASCII_To_BCD(pan.getBytes());
//
//		pwdByte = setxor(bytes2, bytes1);
//		// 获取密码
//		byte[] bytes = new byte[8];
//		for (int i = 0; i < 8; i++) {
//			bytes[i] = (byte) ((pwdByte[i] ^ bytes2[i]) & 0xff);
//		}
//		String desPWD = bcd2Str(bytes);
//		
//		byte[] b = null;
//		try {
//			b = DESUtil.encrypt3(desPWD, bcd2Str(pinKeyByte));
//		} catch (Exception e) {
//			e.printStackTrace();
//			return "";
//		}
